...
export function DocumentPanel({ text, entities, approved, mode, title }: Props) {
  const active = entities
    .map((ent, i) => ({ ent, i }))
    .filter(({ i }) => approved[String(i)] !== false)
    .sort((a, b) => a.ent.start - b.ent.start);

  // Build type counters for pseudonym labels
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <ScrollArea className="flex-1 rounded-lg border border-neutral-200 bg-neutral-50">
        <div
          className="p-5 font-mono text-[13px] leading-7 whitespace-pre-wrap break-words"
